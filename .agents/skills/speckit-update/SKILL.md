...
2. 執行 helper 建立並切換到暫存分支：
   ```powershell
   pwsh .Codex/skills/speckit-update/scripts/speckit-update-helper.ps1 -Mode branch -Issue 755
   ```
...
2. 列出既有 feature 與標題作為分類比對基準：
   ```powershell
   pwsh .Codex/skills/speckit-update/scripts/speckit-update-helper.ps1 -Mode features
   ```
