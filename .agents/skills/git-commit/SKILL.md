...
name: git-commit
description: >
  每當 Codex（或 Copilot）完成任何「實際修改/新增/刪除程式碼或專案檔案」之後，
...
統一本專案所有由 AI agent（Codex、GitHub Copilot）產生之 commit message 風格，
讓開發者能「不思考、直接複製」即可完成 git commit。

> 本規範與根目錄 `AGENTS.md` 之「回應格式規範」搭配使用：
> AGENTS.md 規定「修正/開發完成需說明原因與作法」；本文件規定「該說明結束後，必須再附上一段可複製的 commit message」。
...
> 唯一允許使用 emoji 的位置（其他地方仍維持 AGENTS.md「除非使用者明確要求否則不使用 emoji」之規範）。
...
| `[Codex]` | Codex 相關設定檔變更（`.Codex/`、`AGENTS.md`…）。 |
...
- 至少包含 **一條「原因（Why）」** ：對應 AGENTS.md「說明評估原因」之要求。
...
5. ❌ 使用英文 commit message → 一律繁體中文（與 AGENTS.md 一致）。
6. ❌ 在 commit message 中加上「🤖 Generated with Codex」「Co-Authored-By」之類預設簽名 → 本專案 commit 風格不需要。

---

## 與其他規範的關係

- **AGENTS.md**：總體回應格式規範；本文件是它的延伸落地工具。
