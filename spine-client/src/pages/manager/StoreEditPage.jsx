...
import { updateStore, getStore } from '../../api/manager/store';
...
function StoreEditPage() {
    ...
    const fetchStoreData = async () => {
        try {
            // 優先使用 location.state 中的資料
            if (location.state?.store) {
                setFormData(location.state.store);
                setIsLoading(false);
            } else {
                // 如果沒有，從 API 取得
            ...
            console.error('取得店面資料發生錯誤', error);
            alert('取得店面資料失敗');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await updateStore(updateData);
            ...
            console.error('更新店面錯誤:', error);
            alert('店面更新失敗：' + (error.response?.data?.error || error.message));
        } finally {
    ...
    // 取消
    const handleCancel = () => {
        if (window.confirm('確定要取消編輯？未保存的資料將會遺失。')) {
            navigate('/manager/store/list');
        }
    };
